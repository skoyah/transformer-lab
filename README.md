...
- One tokenizer, the common one: byte-level byte-pair encoding (case kept, any character representable, ▁ = the space byte in front of a word), with 300 merges learned once from a small original English corpus (`js/corpus.js`) — Chapter 1 has a player that performs the merges (count every occurrence, then glue), a tokenise-anything box, and the "why chatbots can't count the r's in strawberry" consequence; prompts are tokenised the same way.
