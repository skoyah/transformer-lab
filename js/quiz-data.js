// js/quiz-data.js — questions for the end-of-chapter 'Check yourself' boxes.
export const QUIZZES = {
  'tokens.html': [
    { q: 'A word the tokenizer has never seen before…', a: ['is skipped — the model cannot use it', 'is cut into smaller pieces it does know', 'is stored whole as one new piece'], ok: 1,
    ...
    { q: 'Why is “strawberry” hard for a chatbot to spell out?', a: ['It is a rare word', 'The model receives pieces, never letters', 'Fruit names are not in the dictionary'], ok: 1,
      why: 'The model sees ticket numbers for pieces, like the ones you saw above; the letters inside a piece are not in its number.' },
  ],
  'embeddings.html': [
  ...
  'ffn.html': [
    { q: 'Why add the attention result onto the original row instead of replacing it?', a: ['To save memory', 'So the token keeps what it already had and attention only proposes a change', 'Because the numbers would be too small otherwise'], ok: 1,
      why: 'That is the residual connection — one of the main reasons deep networks can be trained.' },
  ...
  'output.html': [
    { q: 'The model gave the real next piece a probability of 10%. Its surprise there is about… (see the surprise values in the chapter)', a: ['0.1', '2.3', '10'], ok: 1, why: '−log(0.10) ≈ 2.3. Surprise grows as the probability shrinks.' },
  ...
  'playground.html': [
    ...
    { q: 'Temperature 0 means the model…', a: ['always picks its favourite piece', 'picks a random piece', 'refuses to answer'], ok: 0, why: 'Higher temperature adds randomness; at 0 it always takes the favourite.' },
  ],
};
